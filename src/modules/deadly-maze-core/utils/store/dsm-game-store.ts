...
import { DSMError, DSMStoreError } from '@/src/modules/deadly-maze-core/utils/errors';
...
export class DSMGameStore extends Store<DSMGameState> {
  ...
  getBodies(ids: IBodyState['id'][]) {
    return ids.map((id) => {
      ...
      const molecules = this.getSpaceRegidBodies(state.spaceId, state.moleculeIds).map(
        (rigidBody) => new BodyMolecule({ rigidBody }),
      );
  ...
  saveBodies(entities: BodyModel[]) {
    this.setState((state) => {
      entities.forEach((entity) => {
        ...
        if (!state.bodiesBySpaceId[entity.getState().spaceId])
          state.bodiesBySpaceId[entity.getState().spaceId] = new Set();
