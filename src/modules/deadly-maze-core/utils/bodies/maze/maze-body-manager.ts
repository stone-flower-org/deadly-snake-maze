...
import { DSMError } from '@/src/modules/deadly-maze-core/utils/errors';
import { PhysicsEngine } from '@/src/modules/deadly-maze-core/utils/physics-engine';
import { BodyMolecule, IQueryFuncResult, SpaceModel } from '@/src/modules/deadly-maze-core/utils/store';
...
export class MazeBodyManager extends AbstractBodyManager<MazeBody> {
  ...
  protected _addMazeCellRightWall(body: MazeBody, params: Required<CreateMazeParams>, [x, y]: [number, number]) {
    ...
    body
      .getRootMolecule()
      ?.addAtom(world.createCollider(wallDesc, body.getRootMolecule()?.getBody()), MazeBody.ATOMS.wall.type);
  }
  ...
  protected _addFloor(body: MazeBody, params: Required<CreateMazeParams>) {
    ...
    body
      .getRootMolecule()
      ?.addAtom(world.createCollider(floorDesc, body.getRootMolecule()?.getBody()), MazeBody.ATOMS.floor.type);
  }
  ...
  protected _addBoundaries(body: MazeBody, params: Required<CreateMazeParams>) {
    ...
    body
      .getRootMolecule()
      ?.addAtom(world.createCollider(tWallDesc, body.getRootMolecule()?.getBody()), MazeBody.ATOMS.boundary.type);
    ...
    body
      .getRootMolecule()
      ?.addAtom(world.createCollider(rWallDesc, body.getRootMolecule()?.getBody()), MazeBody.ATOMS.boundary.type);
    ...
    body
      .getRootMolecule()
      ?.addAtom(world.createCollider(bWallDesc, body.getRootMolecule()?.getBody()), MazeBody.ATOMS.boundary.type);
    ...
    body
      .getRootMolecule()
      ?.addAtom(world.createCollider(lWallDesc, body.getRootMolecule()?.getBody()), MazeBody.ATOMS.boundary.type);
  }
  ...
  protected _addMazeExit(body: MazeBody, params: Required<CreateMazeParams>, cell: [number, number]) {
    ...
    const exitDesc = Rapier.ColliderDesc.cuboid(
      MazeBody.ATOMS.exit.size.l,
      MazeBody.ATOMS.exit.size.h,
      MazeBody.ATOMS.exit.size.w,
    );
    ...
    body
      .getRootMolecule()
      ?.addAtom(world.createCollider(exitDesc, body.getRootMolecule()?.getBody()), MazeBody.ATOMS.exit.type);
  }
  ...
  protected _addMazeHunterSpawn(body: MazeBody, params: Required<CreateMazeParams>, cell: [number, number]) {
    ...
    const hunterSpawnDesc = Rapier.ColliderDesc.cuboid(
      MazeBody.ATOMS.hunterSpawn.size.l,
      MazeBody.ATOMS.hunterSpawn.size.h,
      MazeBody.ATOMS.hunterSpawn.size.w,
    );
    ...
    body
      .getRootMolecule()
      ?.addAtom(
        world.createCollider(hunterSpawnDesc, body.getRootMolecule()?.getBody()),
        MazeBody.ATOMS.hunterSpawn.type,
      );
  }
  ...
  protected _addMazePreySpawn(body: MazeBody, params: Required<CreateMazeParams>, cell: [number, number]) {
    ...
    const preySpawnDesc = Rapier.ColliderDesc.cuboid(
      MazeBody.ATOMS.preySpawn.size.l,
      MazeBody.ATOMS.preySpawn.size.h,
      MazeBody.ATOMS.preySpawn.size.w,
    );
    ...
    body
      .getRootMolecule()
      ?.addAtom(world.createCollider(preySpawnDesc, body.getRootMolecule()?.getBody()), MazeBody.ATOMS.preySpawn.type);
  }
  ...
  protected _calcWallSize(_: Required<CreateMazeParams>) {
    return [
      MazeBody.ATOMS.cell.size.w + MazeBody.ATOMS.wall.size.w,
      MazeBody.ATOMS.wall.size.h,
      MazeBody.ATOMS.wall.size.w,
    ];
  }
