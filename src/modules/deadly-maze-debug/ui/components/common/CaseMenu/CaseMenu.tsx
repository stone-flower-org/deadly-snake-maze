...
import { DEADLY_MAZE_DEBUG_CASE_PAGE_ID } from '@/src/modules/deadly-maze-debug/constants';
import { CASES } from '@/src/modules/deadly-maze-debug/utils/cases';
    ...
    <StyledWrapper>
      ...
      <StyledList>
        {cases.map(({ id, name }) => (
          <ListItem key={id}>
            <Link
              to={app.getService('routesStore').generateFullPathById(DEADLY_MAZE_DEBUG_CASE_PAGE_ID, {
                params: { caseId: id },
              })}
            >
              {name}
