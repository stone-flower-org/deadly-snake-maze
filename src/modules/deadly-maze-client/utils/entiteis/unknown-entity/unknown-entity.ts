...
import {
  ...
  BodyMoleculeSync,
  IBodySync,
} from '@/src/modules/deadly-maze-client/utils/entiteis/utils';
import { unknownGeometry } from '@/src/modules/deadly-maze-client/utils/geometries';
...
export class UnknownEntity extends AbstractBodyEntity {
  ...
  static registerEntity(sceneRenderer: SceneRenderer) {
    sceneRenderer
      .getEntityMap()
      .set(this.ENTITIES.unknownAtom, (body: BodyAtom) => {
      ...
      .set(
        ...
        (body: BodyMolecule) =>
          new UnknownEntity({
            bodyId: body.getId(),
            view: new THREE.Group(),
            sync: new BodyMoleculeSync({ sceneRenderer }),
          }),
      );
  }
