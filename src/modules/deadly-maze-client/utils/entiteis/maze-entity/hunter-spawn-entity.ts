...
export class HunterSpawnEntity extends AbstractBodyEntity<BodyAtom> {
    ...
    return (atom: BodyAtom) => {
      ...
      if (!geometry) {
        geometry = new THREE.BoxGeometry(
          ...
          h,
          d,
          w * HunterSpawnEntity.OPTIONS.geometry.segmentsDensity,
          h * HunterSpawnEntity.OPTIONS.geometry.segmentsDensity,
          d * HunterSpawnEntity.OPTIONS.geometry.segmentsDensity,
        );
      }

      const view = new THREE.Mesh(geometry, unknownMaterial); // TODO: delete me

      return new HunterSpawnEntity({ bodyId: atom.getId(), view });
    };
  }
