...
export class PreySpawnEntity extends AbstractBodyEntity<BodyAtom> {
    ...
    return (atom: BodyAtom) => {
      ...
      if (!geometry) {
        geometry = new THREE.BoxGeometry(
          ...
          h,
          d,
          w * PreySpawnEntity.OPTIONS.geometry.segmentsDensity,
          h * PreySpawnEntity.OPTIONS.geometry.segmentsDensity,
          d * PreySpawnEntity.OPTIONS.geometry.segmentsDensity,
        );
      }

      const view = new THREE.Mesh(geometry, unknownMaterial); // TODO: delete me

      return new PreySpawnEntity({ bodyId: atom.getId(), view });
    };
  }
